...
saveas(f1, [modelPath, '\figures\gradCAM\s', int2str(subject), '\', int2str(win), '.png']);
close(f1)
...
saveas(f2, [modelPath, '\figures\boxInBox\s', int2str(subject), '\', int2str(win), '.png']);
close(f2)
...
saveas(f3, [modelPath, '\figures\GT\s', int2str(subject), '\', int2str(win), '.png']);
close(f3)
end
end
end
