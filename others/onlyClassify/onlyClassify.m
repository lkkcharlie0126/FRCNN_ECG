...
for i = 1:size(frcnnTrainner.T_used, 1)
    ...
    for j = 2:7
        for k = 1:size(frcnnTrainner.T_used{i, j}{1}, 1)
            ...
            imBeat = im(beatLoc(2):beatLoc(2)+beatLoc(4), beatLoc(1):beatLoc(1)+beatLoc(3), :);
            
            imSize = size(imBeat);
            %% new input layer
            newLayers_1 = removeLayers(newLayers_1, 'input_1');
            newLayers_1 = addLayers(newLayers_1, imageInputLayer(imSize, 'Name', 'input_1', 'Mean', inputMeanLayer_1));
            newLayers_1 = connectLayers(newLayers_1, 'input_1', 'conv1');
            
            % analyzeNetwork(newLayers)
            newNet = assembleNetwork(newLayers_1);

            activate = activations(newNet,imBeat,'activation_19_relu');
            activateSize = size(activate);
            roiOut = visiongpuROIMaxPoolingForward(gpuArray(activate), double([1; 1; activateSize(1); activateSize(2); 1]), 8, 8);

            output = activations(newNet2, roiOut,'rcnnClassification');
            output = reshape(gather(output), [1, 7]);
